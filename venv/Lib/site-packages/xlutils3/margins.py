...
def ispunc(c, s=set(string.punctuation)):
...
def check_file(fname, verbose, do_punc=False, fmt_info=0, encoding='ascii', onesheet=''):
        ...
        if verbose >= 3:
            ...
            for rowx in xrange(sheet.nrows):
                ...
                if cell.ctype != XL_CELL_EMPTY:
                    print (cellname(rowx, lastcolx), "(",rowx, lastcolx,"):", cell.ctype, cell.value)
        if (verbose
            ...
            print (shx, sheet.nrows, sheet.ncols,
                    ngoodrows, ngoodcols, pctwaste, sheet_density_pct_s, shname_enc)#"sheet #%2d: RxC %5d x %3d => %5d x %3d; %4.1f%% waste%s (%s)" % \
    ...
    if totold:
        ...
        pctwaste = 0.0
    print (totold, totnew, pctwaste)#"%d cells => %d cells; %4.1f%% waste" % (totold, totnew, pctwaste)
