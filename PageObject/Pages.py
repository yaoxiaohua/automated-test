...
class PagesObjects:
    ...
    def operate(self):
        ...
        for item in self.testCase:
            ...
            if item.get("operate_type", "0") == be.SWITCH_DEFAULT_FRAME:
                self.driver.switch_to.default_content()
                print("--切换默认iframe---")
            if item.get("operate_type", "0") == be.SWITCH_PARENT_FRAME:
                self.driver.switch_to.parent_frame()
                print("--切换默认iframe---")
            if item.get("operate_type", "0") == be.CLICK_EXCEL_ELE:
                excel_ele = read_append('Record')
                # xpath_ele = "//android.widget.TextView[@text='"+excel_ele+"']/../../../../.."
                # xpath_ele = "//a[contains(text(),'%s')]/../ preceding-sibling::td[4]/label/input" % excel_ele
                xpath_ele = "//td[contains(text(),'%s')]/ preceding-sibling::td[2]/label/input" % excel_ele
                print("--打印值%s---" % xpath_ele)
                click_ele = self.driver.find_element_by_xpath(xpath_ele)
                click_ele.click()
                print("--点击excel存储对应元素---")
        ...
        if self.isOperate:
            for item in self.test_check:
                ...
                if item.get("check", be.DEFAULT_CHECK) == be.COMPARE and self.is_get and resp["text"]\
                    ...
                    result = False
                    m = get_error({"type": be.COMPARE, "current": resp["text"], "history": self.get_value})
                    print(m)
                    self.testInfo[0]["msg"] = m
                    break
